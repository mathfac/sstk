...
AssetsDb.prototype.query = function(params, callback) {
    ...
    if (query === '' || query === '*:*') {
        ...
        if (queryTerms.length === 1 && queryTerms[0].startsWith('fullId:')) {
            ...
            var assetInfo =
                this.getAssetInfo(queryTerms[0].substring('fullId:'.length));
            var docs = [];
            if (assetInfo) {
                docs.push(assetInfo);
            }
            var data = {
                response : {docs : docs, start : 0, numFound : docs.length}
            };
            callback(null, data);
        } else {
            var filter;
...
AssetsDb.prototype.__loadAssetInfoFromAssetIdList = function(assetGroup, data) {
    ...
    var assetInfos = lines.map(function(s) { return {id : s}; });
...
AssetsDb.prototype.__updateAssetInfo = function(assetGroup, m) {
    if (assetGroup) {
        ...
        if (assetGroup.assetFields && _.isArray(assetGroup.assetFields)) {
            var loadInfo =
                assetGroup.getLoadInfo(m[this.assetIdField], m['format'], m);
...
AssetsDb.prototype.__loadAssetInfoFromCsvData = function(assetGroup, data) {
    var parsed = IOUtil.parseDelimited(data, {
        header : true,
        skipEmptyLines : true,
        dynamicTyping : function(fieldname) {
            // Make sure id is treated as a string, but other fields are
            // dynamically typed
            if (fieldname === 'id') {
                return false;
            } else {
    ...
    var arrayFields = {
        ...
        scene : [
            'datasets', 'modelIds', 'modelCats', 'modelNames', 'modelTags',
            'roomIds', 'roomTypes', 'origRoomTypes'
        ]
    };
...
AssetsDb.prototype.loadAssetInfoFromData = function(assetGroup, data, filename,
                                                    options) {
    options = options || {};
    var assetInfos;
    ...
    if (options.mode === 'merge' && options.assetField) {
        ...
        for (var i = 0; i < assetInfos.length; i++) {
            ...
            _.set(asset, options.assetField,
                  _.omit(m, [ assetIdField, 'fullId', 'source' ]));
        }
    } else {
        var assetIdToInfo = {};
        for (var i = 0; i < assetInfos.length; i++) {
        ...
        this.fields =
            DataUtils.extractFieldsFromData(this.assetInfos, this.fieldOptions);
    }
};

AssetsDb.prototype.loadAssetInfo = function(assetGroup, file, callback,
                                            options) {
    var scope = this;
    var loader = new AssetLoader();
    return loader.loadErrorFirst(file, undefined, function(err, data) {
        if (!err) {
            scope.loadAssetInfoFromData(assetGroup, data, file.name || file,
                                        options);
        }
        if (callback) {
            callback(err, scope);
