...
var __optsToIgnore = [
    'renderer', 'simpleRenderer', 'assetManager', 'net', 'wav', 'fs',
    'bufferType', 'rendererType', 'rendererFactory'
];
 ...
 *   (default = `{rgb: true, depth: false, normal: false, objectType: false,
 *objectId: false, roomType: false, roomId: false, forces: true, audio:
 *false}`).
 * @param [opts.unit=1] {number} Unit to use for space (in meters).
 * @param [opts.timeStep=0.1] {number} Amount of time (in secs) a tick/step
 *corresponds to.
 * @param [opts.assetManager] {AssetManager} For loading assets (will be created
 *if not specified).
 * @param [opts.assetCacheSize=100] {int} Size of asset cache
 * @param [opts.outputDir] {string} Output directory for debug output (e.g.
 *screenshots and audio output)
 ...
 * @param [opts.start] {Object|string} Start specification (use `random` for
 *random start)
 * @param [opts.goal] {Object|string} Goal specification (use `random` for
 *random goal)
 * @param [opts.audio] {Object} Options for audio simulator
 * @param [opts.navmap] {Object} Options for navigation map (use `{
 *recompute=true }` for force recomputation of navigation maps
 ...
 * @param [opts.useSky=true] {boolean} Whether to use grass and sky for outside
 *environment.
 * @param [opts.actionTraceLog] {string|boolean} True to use log action traces
 *to file.  If string, used to specify filename of action trace.
 *        By default, action trace is saved to `outputDir/action_trace.csv`.
 * @param [opts.actionTraceLogFields] {string[]} List of additional fields to
 *include in action trace log. Supported fields are: `forces`.
 * @param opts.renderer {Renderer} Renderer for rendering rgb (can have fancy
 *effects)
 * @param [opts.rendererType] {Class} Renderer class (used for creating more
 *renderers)
 * @param [opts.bufferType] {Buffer} node.js Buffer (webpack gets some other
 *Buffer) for audio simulator
...
function Simulator(opts) {
    ...
    console.log('Creating Simulator with options',
                _.omit(opts, __optsToIgnore));
    this.__init(opts);
}
...
Simulator.prototype.__init = function(opts) {
    ...
    if (opts.visualizeSensors != undefined) {
        _.each(opts.sensors,
               function(s) { s.visualize = opts.visualizeSensors; });
    }
    ...
    // this.__rendererConfig = opts.rendererConfig || {};  // Default renderer
    // config
    this.renderer = opts.renderer;
    // TODO: should the renderer factory be passed in?
    this.__rendererFactory = new RendererFactory({
        rendererType :
            opts.rendererType || Renderer, // What class to use for rendering
        renderers : {'main' : this.renderer},
        configSets : {
            'simple' : {
                isOffscreen : true,
    ...
    this.__collisionProcessor =
        CollisionProcessorFactory.createCollisionProcessor(
            opts.collisionDetection);
    this.state = new SimState(_.defaults(
        {rendererFactory : this.__rendererFactory, sampler : this.sampler},
        opts));
    this.state.collisionProcessor =
        CollisionProcessorFactory.createCollisionProcessor(
            _.defaults(
                {mode : 'raycast'},
                opts.collisionDetection) // for sampling start configurations
                                         // (avoids NavScene co-dependency)
            );

    this.__outputDir = opts.outputDir || '.'; // Output directory for debug
    this.__resetCnt = 0;                      // Reset count
    this.__simOperations = new SimOperations({simulator : this});
    ...
    if (actionTraceLog) {
        ...
        var actionTraceFields =
            [ 'tick', 'px', 'py', 'pz', 'rotation', 'actions', 'actionArgs' ];
        if (opts.actionTraceLogFields) {
            ...
            for (var i = 0; i < opts.actionTraceLogFields.length; i++) {
                ...
                if (field === 'forces') {
                    ...
                    console.warn('Skipping unknown actionTraceLogField: ' +
                                 field);
                }
            }
        }
        this._actionTrace = new ActionTraceLog({
            ...
            logfile :
                actionTraceLog
                    ? {filename : actionTraceLog, fields : actionTraceFields}
                    : null
        });
    ...
    if (opts.observations.audio && opts.audio) {
        if (!opts.net) {
            console.error(
                'Attempting to start audio simulator without net.  Not using Audio!');
        } else {
            this.audioSim = new AudioSimulator(
                _.merge(Object.create(null), opts.audio || {}, {
                    scene : opts.scene,
                    net : opts.net,
                    fs : opts.fs,
                    wav : opts.wav,
...
Simulator.prototype.__getCameraFrames = function() {
    ...
    if (cameraSensors) {
        ...
        _.forEach(cameraSensors, function(sensor, name) {
            // Make sure this sensor is active and observations for it is
            // enabled
            // TODO: Should observation be based on sensor type or sensor
            // name???
            if (scope.opts.observations[sensor.config.name]) {
                results[sensor.config.name] =
                    sensor.getFrame(scope.state.sceneState);
            }
        });
        return results;
    }
...
Simulator.prototype.__getAudioBuffer = function(callback) {
    if (this.audioSim) {
        this.audioSim.getAudioBuffer(
            {
              startTime :
                  Math.round(this.state.prevTicks * this.timeStep * 1000),
...
/**
 * Acts for nframes time step with given action, updates simulator, and returns
 * observations.
 * @param {Object|Object[]} [action] What action or sequence of actions to take.
 * Each action should have a `name`
 *   and necessary input parameters.  Specific actions supported by Agent is
 * Agent dependent.
 * @param {int} [nframes=1] How many times to repeat the action (and number of
 * ticks to increment).
 * @param {Function} [callback] Error first callback with object containing the
...
Simulator.prototype.step = function(action, nframes, callback) {
    ...
    async.waterfall(
        [
          ...
          function(cb) { scope.update(cb); },
          function(cb) { scope.getObservations(cb); }
        ],
        function(err, result) {
            if (err) {
...
Simulator.prototype._logAction = function(actionname, actionArgs) {
    if (this._actionTrace) {
        ...
        var rec =
            [ this.state.ticks, p.x, p.y, p.z, r, actionname, actionArgs ];
...
/**
 * Tells agent to take action.  Action is tracked in action log (before action)
 * and time is updated.
...
/**
 * Updates the scene state after actions.  Collisions and forces are computed.
 * Audio is notified of updated position of agent.
...
Simulator.prototype.update = function(callback) {
    ...
    var agent = this.getAgent();
    var sceneBBox =
        Object3DUtil.getBoundingBox(this.state.sceneState.fullScene);
    var dt = this.opts.timeStep;
    agent.setPredictedVs(dt);
    if (this.opts.observations.forces) {
        this.__collisionProcessor.computeSensorForces(this.state.sceneState,
                                                      this.state.agent, dt);
    }
    var response = this.__collisionProcessor.checkForCollision(
        this.state.sceneState, agent, dt);
    // var beforePosition = JSON.stringify(agent.position);
    // var beforeCanStand =
    // this.__collisionProcessor.isPositionAgentCanStandAt(this.state.sceneState,
    // agent, agent.getAgentFeetAt());
    if (response.collision) {
        // check if first collision response causes second collision, and
        // hackishly kill all velocity
        var secondResponse = this.__collisionProcessor.checkForCollision(
            this.state.sceneState, agent, dt);
        if (secondResponse.collision) {
            agent.velocity.set(0, 0, 0);
        } else {                  // apply first collision response
            agent.velocity.y = 0; // ignore vertical displacements
            agent.position.addScaledVector(agent.velocity,
                                           dt * Constants.metersToVirtualUnit);
        }
        agent.rotation += agent.angularVelocity * dt;
    } else if (!sceneBBox.contains(agent.position)) {
        // TODO: double check this logic, sometimes agent accidentally goes out
        // and can't get back in!
        // check for collisions with scene boundary and hackily push agent back
        // in
        ...
        delta.y = 0;
        delta.normalize();
        agent.position.addScaledVector(delta,
                                       dt * Constants.metersToVirtualUnit);
        agent.rotation += agent.angularVelocity * dt;
    } else {
        agent.update(dt);
    ...
    if (this.audioSim) {
        ...
        var receivers =
            this.state.getAudioReceivers({op : 'update', baseId : 0});
...
Simulator.prototype.getGoalObservations = function(opts, callback) {
    ...
    var agentPosition = {
        ...
        rotation : agent.angle,
        isTemporary : true
    }; // isTemporary indicates other stuff don't really change
    ...
    async.map(
        ...
        function(goal, cb) {
            ...
            goalPos.position.y =
                agent.position.y; // TODO: handle if goal is on different floor
            ...
            if (st === 'camera') {
                ...
                if (audioSim) {
                    sensorMetadata[s.name] = {
                        ...
                        shape : [
                            s.position.length,
                            audioSim.opts.samplingRate * scope.timeStep, 1
                        ],
                        dataType : audioSim.opts.datatype,
                        dataRange : (audioSim.opts.datatype === 'float32')
                                        ? [ -1, 1 ]
                                        : [ 0, 255 ]
                    };
                }
 ...
 * @param {Function} [callback] Error first callback with object containing
 * useful stuff.
 */
Simulator.prototype.getObservations = function(callback) {
    ...
    if (this.navmap) {
        ...
        var roomIntersect = this.state.sceneState.getIntersectedRoomAt(
            agentPosition, this.opts.scene.level);
        if (roomIntersect) {
    ...
    var data = {
        observation : {
            ...
            measurements : {
                distance_to_goal :
                    offsetsToGoal.map(function(x) { return x.distance; }),
                offset_to_goal : _.flatMap(
                    offsetsToGoal, function(x) { return x.offset.toArray(); }),
                direction_to_goal :
                    _.flatMap(offsetsToGoal,
                              function(x) {
                                  var dir = x.offset.normalize();
                                  return [
                                      dir.x, 0, -dir.z
                                  ]; // note -z flip due to agent front in -z
                              })
            },
            roomInfo : roomInfo
        },
    ...
    if (this.state.navscene) {
        ...
        data.observation.measurements.shortest_path_to_goal = {
            distance : (path.distance !== undefined) ? path.distance : null,
          ...
          function(cb) {
              ...
              scope.__getAudioBuffer(function(audio) {
                  if (!audio) {
                      ...
                      // var buf = scope.opts.wav.encode([samples], {
                      // sampleRate: 44100, float: false, bitDepth: 16 });
                      // scope.opts.fs.writeFileSync('test.wav', buf, {
                      // encoding: null });
...
Simulator.prototype.prepareTopDownMapProjection = function() {
    ...
    var renderer = this.__rendererFactory.getRenderer(
        'simple-2000x2000',
        {configSet : 'simple', width : 2000, height : 2000});
    var bbox = Object3DUtil.getBoundingBox(sceneState.scene);
    var agent = this.getAgent();
    var agentFeetPosition = this.getAgent().getAgentFeetAt();
    var floorHeight = agentFeetPosition.y +
                      this.__collisionProcessor.traversableFloorHeight *
                          Constants.metersToVirtualUnit;
    var topHeight =
        agentFeetPosition.y + agent.height * Constants.metersToVirtualUnit;

    // renderer.renderer.clippingPlanes = Constants.EmptyArray;
    this.__viewProjectionGenerator =
        this.__viewProjectionGenerator || new ViewProjectionGenerator({
            renderer : renderer,
    ...
    var view = this.__viewProjectionGenerator.viewGenerator.getView({
        name : 'view',
        target : viewBBox,
        viewIndex : 4 /* top view */,
        pixelWidth : 0.01
    });
    ...
    this.topDownMapProjection = this.__viewProjectionGenerator.generate(
        sceneState.scene, {view : view, clipToBox : viewBBox},
        this.topDownMapProjection);
    ...
    if (this._actionTrace) {
        ...
        for (var i = 0; i < goals.length; i++) {
            ...
            this._actionTrace.push([
                this.state.ticks, p.x, p.y, p.z, r, 'goal', goals[i].objectId
            ]);
        }
    }
};
...
/**
 * Reset simulator to a new episode.  Start/goals are reinitialized and agent
 * moved to start.
 */
...
Simulator.prototype.seed = function(seed) {
    // Sets seed used in environments random number generator (for
    // reproducibility)
...
Simulator.prototype.__updateSkyGround = function(sceneState) {
    if (this.opts.useSky) {
        ...
        this._skyGround.position.copy(Object3DUtil.getBBoxFaceCenter(
            sceneState.fullScene, Constants.BBoxFaceCenters.BOTTOM));
    }
};
...
Simulator.prototype.__loadScene = function(opts, callback) {
    ...
    var intensity = this.assetManager.enableLights
                        ? 0.5
                        : 2.0; // set low if interior lights turned on
    var light = new THREE.HemisphereLight(0xffffff, 0x202020, intensity);
    // scene.add(light);
    // scene.add(this.getAgent().getObject());
    if (window) {
        window.scene = scene;
    } // NOTE: for debugging with three.js inspector
    var scope = this;
    ...
    if (this.opts.navmap && this.opts.navmap.recompute !== true) {
        ...
        // TODO: enforce recompute false = no recomputation, only use
        // precomputed
        //               recompute undefined = use precomputed, recompute as
        //               needed
    }
    ...
    this.assetManager.loadAssetAsScene(sceneOpts, function(err, sceneState) {
        ...
        console.log('Loaded ' + sceneState.getFullID() + ' bbdims: [' +
                    bbdims.x + ',' + bbdims.y + ',' + bbdims.z + ']');
        scope.scene = scene;
        ...
        if (scope.audioSim) {
            ...
            var receivers =
                scope.state.getAudioReceivers({op : 'create', baseId : 0});
            // Stupid audio simulator requires sound sources be inside walls
            var sources = scope.state.getAudioSources({
                op : 'create',
                baseId : receivers.length,
                keepInsideWalls : true
            });
            scope.audioSim.configure(
                {endpoints : _.concat(receivers, sources)});
...
Simulator.prototype.getAgent = function() { return this.state.agent; };

// Returns simulator state
Simulator.prototype.getState = function() { return this.state; };
