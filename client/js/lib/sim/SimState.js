...
function SimState(opts) {
    this.opts = {
        sampleFloor : {nsamples : 300, maxIters : 100},
        sampleValidStartGoal :
            {maxStartSamples : 500, maxGoalSamples : 1}
    };
...
SimState.prototype.__init = function() {
    var opts = _.pick(this.opts, [
        'agent', 'semanticEncodings', 'sensors', 'rendererFactory',
        'colorEncoding'
    ]);
...
SimState.prototype.reset = function(sceneState) {
    ...
    // TODO: convert from this.opts.scene.level to loaded level index for SUNCG
    // scenes that can be loaded at one level at a time
    this.level = (this.opts.scene.level != undefined)
                     ? sceneState.getLevelByIndex(this.opts.scene.level)
                     : null;
    ...
    if (this.opts.unit) {
        var scaleBy = this.opts.unit
                          ? (sceneState.info.defaultUnit / this.opts.unit)
                          : 1.0;
    ...
    if (this.opts.navmap) {
        ...
        if (!this.navscene || this.navscene.sceneState !== sceneState) {
            ...
            if (this.opts.navmap.refineGrid) {
                var defaultOpts = {
                    radius : this.agent.radius * Constants.metersToVirtualUnit,
                    clearance : this.agent.radialClearance *
                                    Constants.metersToVirtualUnit
                };
                refineGridOpts =
                    _.isPlainObject(this.opts.navmap.refineGrid)
                        ? _.defaults(this.opts.navmap.refineGrid, defaultOpts)
                        : defaultOpts;
            }
            this.navscene = new NavScene({
                ...
                cellAttributes : _.keyBy(
                    [
                      new NavScene.CellAttribute('floorHeight', Float32Array,
                                                 {dataType : 'continuous'}),
                      new NavScene.CellAttribute('roomIndex', Uint16Array,
                                                 {dataType : 'categorical'}),
                      new NavScene.CellAttribute('occupancy', Uint8Array,
                                                 {dataType : 'categorical'})
                    ],
                    'name'),
                ...
                cellSize :
                    (this.opts.navmap.cellSize || (this.agent.radius / 2)) *
                        Constants.metersToVirtualUnit,
                metadata : {
                    agentHeight : this.agent.height,
                    agentRadius : this.agent.radius,
                    traversableFloorHeight :
                        scope.collisionProcessor.traversableFloorHeight,
                    useCellFloorHeight : useCellFloorHeight
                },
                baseTileHeight :
                    scope.collisionProcessor.traversableFloorHeight *
            ...
            if (this.navscene.isGridValid()) {
                ...
                if (Constants.isBrowser) {
                    ...
                    console.log('shortestPath',
                                this.navscene.getShortestPath());
                }
            } else {
                console.warn(
                    'Cannot create navscene!  Please check your configuration');
                this.navscene =
                    null; // NOT able to create navscene successfully
            }
        }
    }
};
...
SimState.prototype.ensureValidPath = function(opts) {
    opts = _.defaults(Object.create(null), opts || {},
                      this.opts.sampleValidStartGoal);
    var ngsamples = 0;
    ...
    if (!hasPath) {
        ...
        while (!hasPath && ngsamples < opts.maxGoalSamples) {
            ...
            if (_.isFinite(opts.maxStartSamples)) {
                ...
                // Use navscene sampler that searches the entire grid.  NOTE:
                // check performance for large scenes
                // As by product will get cost to goal for all tiles
                // This is good if we have more constraints on properties of the
                // path to the target (like number of doors, number of steps,
                // etc).
                var sampleOpts = _.defaults({
                    /*nsamples: 1,*/ sampler : this.opts.sampler,
                    scorer : 'path',
                    goals : this.goals,
                    mapState : {map : []}
                },
                ...
                if (sample) {
                    this.start =
                        this.__updateNavSceneSample(sample, this.opts.rng);
                    this.agent.moveTo(this.start);
    ...
    if (!hasPath) {
        console.warn('No path between start and goals!', this.start,
                     this.goals);
    }
};
 ...
 * @param goal {string|Object} Goal specification ('random' to randomly sample
 * valid goal point)
 ...
 * @param [goal.select] {string} From valid goals, select `closest`, `random`.
 * All goals are allowed if not specified
 */
SimState.prototype.setGoal = function(goal) { this.opts.goal = goal; };

/**
 * Fetch configuration for audio sources - position of audio sources will be
 * centroid of goal, audio file is also audio associated with goal.
 * @param opts Options for creating audio source configuration
 * @param opts.op Operation to perform on the audio source (`create`, `update`,
 * or `remove`)
 * @param opts.baseId [int] Audio endpoints are given an unique integer id.  The
 * baseId specified the next id to start from.
 * @param [opts.keepInsideWalls=false] {boolean} If true, will shrink the wall
 * bounding box by a bit and push all audio
...
/**
 * Fetch configuration for audio receivers - position of audio receivers is
 * based on agent audio sensors.
 * @param opts Options for creating audio receiver configuration
 * @param opts.op Operation to perform on the audio receiver (`create`,
 * `update`, or `remove`)
 * @param opts.baseId [int] Audio endpoints are given an unique integer id.  The
 * baseId specified the next id to start from.
 * @return {sim.AudioSimulator.AudioEndpointConfig[]}
    ...
    _.each(audioSensors, function(sensor, name) {
        ...
        for (var i = 0; i < sensor.position.length; i++) {
            ...
            var receiver = {
                ...
                type : sensorType,
                position : {
                    x : sensorPosition.position.x,
                    z : sensorPosition.position.z
                },
                direction : {
                    x : sensorPosition.direction.x,
                    z : sensorPosition.direction.z
                }
            };
...
SimState.prototype.getGoals = function() { return this.goals; };

SimState.prototype.getSerializableGoals = function() {
    if (!this.__serializableGoals && this.goals) {
        this.__serializableGoals = _.map(
            this.goals, function(g) { return _.omit(g, [ 'modelInstance' ]); });
    }
 ...
 * @returns {Array<{position: THREE.Vector3, room: ?string, angle: ?number,
 * bbox: ?geo.BBox, objectId: ?string, modelInstance: ?model.ModelInstance,
 * initialOffsetFromAgent: ?THREE.Vector3, audioFile: ?string}>}
 */
SimState.prototype.computeGoals = function() {
    console.log("Computing Goals");
    var scope = this;
   ...
   if (goalsSpec.positions != null) {
      for (var i = 0; i < goalsSpec.positions.length; ++i) {
            var tmp_vec = new THREE.Vector3();
            tmp_vec.x = goalsSpec.positions[i][0];
            tmp_vec.y = goalsSpec.positions[i][1];
            tmp_vec.z = goalsSpec.positions[i][2];

            goals.push(JSON.parse(JSON.stringify(goalsSpec)));
            goals[i].position = tmp_vec;
    ...
    if (goalsSpec.position && goalsSpec.roomIds == null) {
        if (goalsSpec.position === 'random') {
            ...
            var tmp_vec = new THREE.Vector3();
            tmp_vec.x = goalsSpec.position[0];
            tmp_vec.y = goalsSpec.position[1];
            tmp_vec.z = goalsSpec.position[2];

            goals.push(JSON.parse(JSON.stringify(goalsSpec)));
            goals[0].position = tmp_vec;
        }
        // TODO room
        // TODO roomType
   ...
   // helper for converting modelInstance to goal entry
    var modelInstanceToGoalFunc = function(mi) {
        ...
        var offsetFromAgent =
            scope.agent.worldToLocalPositionNoScaling(closestPoint);
    ...
    var target_obj_center = null;
    if (goalsSpec.target_pos) {
        var target_center = new THREE.Vector3();
        target_center.fromArray(goalsSpec.target_pos[0]);
    }

    var roomToGoalFunc = function(room) {
        var bbox = Object3DUtil.getBoundingBox(room);
        var sample =
            scope.__sampleFloor(room, scope.opts.sampleFloor, target_center);
    ...
    if (goalsSpec.objectIds &&
        goalsSpec.objectIds.length > 0) { // goals specified by object ids
        ...
        var mInstsOfObjectId =
            this.sceneState.findModelInstances(matchObjectId);
        goals = _.map(mInstsOfObjectId, modelInstanceToGoalFunc);
    } else if (goalsSpec.modelIds &&
               goalsSpec.modelIds.length > 0) { // goals specified by model ids
        var matchId = function(mi) {
            return goalsSpec.modelIds.indexOf(mi.model.getFullID()) >= 0 &&
                   mi.object3D.visible;
        };
        var mInstsOfId = this.sceneState.findModelInstances(matchId);
        goals = _.map(mInstsOfId, modelInstanceToGoalFunc);
    } else if (goalsSpec.categories &&
               goalsSpec.categories.length >
                   0) { // goals specified by categories
        var matchCat = function(mi) {
            return mi.model.hasCategoryIn(goalsSpec.categories) &&
                   mi.object3D.visible;
        };
        ...
        var rooms =
            this.sceneState.getRoomsOrHouseRegions(this.level, function(room) {
        ...
        var rooms = this.sceneState.getRoomsOrHouseRegions(
            this.level,
    ...
    if (goalsSpec.select === 'random') {
        ...
        goals = _.sortBy(
            goals, function(g) { return g.initialOffsetFromAgent.length(); });
        if (goals.length > 0) {
            goals = [ goals[0] ];
        }
    }
    ...
    // TODO: Implement me!!!
    // Support observer option for having a specific position  that is next to
    // the selected goals
    if (goalsSpec.selectObserverPosition) {
        // Currently just do it randomly, in the future can be enhanced to have
        // options of how far away the observer should be positioned
    ...
    if (goals.length === 0) {
        console.error('Could not select a goal given spec ' +
                      JSON.stringify(goalsSpec));
    }
...
SimState.prototype.offsetsToGoals = (function() {
    ...
    return function() {
        ...
        var offsets = _.map(goals, function(g) {
            var closestPoint = g.bbox && scope.opts.goal.dist_from_bbox
                                   ? g.bbox.closestPoint(agent.position, tmp)
                                   : g.position;
            var positionInAgentSpace =
                agent.worldToLocalPositionNoScaling(closestPoint);
            return {
                // distance: positionInAgentSpace.length(),
                distance :
                    Math.sqrt(positionInAgentSpace.x * positionInAgentSpace.x +
                              positionInAgentSpace.z * positionInAgentSpace.z),
...
SimState.prototype.isPositionAgentCanStandAt = function(pFeet) {
    return this.collisionProcessor.isPositionAgentCanStandAt(this.sceneState,
                                                             this.agent, pFeet);
};
...
SimState.prototype.getPositionMetadata = (function() {
    ...
    return function(pFeet, opts) {
        ...
        var roomIntersect =
            this.sceneState.getIntersectedRoomAt(pFeet, opts.level);

        var floorHeight;
        if (roomIntersect) {
            // Height of floor at the cell
            metadata.roomIndex = roomIntersect.object.userData.index +
                                 1; // Add 1 so 0 is safely unknown
            floorHeight = this.sceneState.getFloorHeight(roomIntersect.object);
        } else {
            var groundIntersect =
                this.sceneState.getIntersectedGroundAt(pFeet, opts.level);
            if (groundIntersect) {
                floorHeight =
                    this.sceneState.getFloorHeight(groundIntersect.object);
            }
        }
        if (floorHeight == undefined && opts.estimateFloorHeight) {
            // console.log('estimateFloorHeight', opts.estimateFloorHeight);
            // No floor height - try to estimate from of room floors around this
            // one
            var estimatedFloorHeight = SceneUtil.estimateFloorHeight(
                opts.level, pFeet, opts.estimateFloorHeight.numSamples,
            ...
            if (estimatedFloorHeight != undefined) {
                ...
                var thresh = opts.estimateFloorHeight.maxVerticalDist +
                             opts.estimateFloorHeight.verticalOffset;
                pFeetAdjusted.y = estimatedFloorHeight +
                                  opts.estimateFloorHeight.verticalOffset;
                if (this.collisionProcessor.isPositionInsideScene(
                        this.sceneState, pFeetAdjusted, thresh)) {
        ...
        if (opts.useCellFloorHeight) {
            if (metadata.floorHeight != null &&
                _.isFinite(metadata.floorHeight)) {
        ...
        if (!opts.needFloorHeight || _.isFinite(metadata.floorHeight)) {
            ...
            cost = this.collisionProcessor.getPositionCost(
                this.sceneState, this.agent, pFeetAdjusted, opts);
        }
        metadata.cost = cost;
...
SimState.prototype.sample = function(opts) {
    ...
    var sampled = this.__sampleRoomFloor(
        _.defaults({}, opts || {}, this.opts.sampleFloor));
    var roomId;
    var position;
    if (sampled) {
    ...
    return {room : roomId, position : position, angle : angle};
};
...
SimState.prototype.sampleStart = function() {
    ...
    if (goalSpec && goalSpec.minRooms && numRooms > 1) {
        var goalRoomIds =
            _.filter(_.map(this.goals, function(g) { return g.room; }),
                     function(x) { return x; });
        return this.sample({
            filterRooms : function(r) {
                return goalRoomIds.indexOf(r.userData.id) < 0;
            }
        });
    } else {
        return this.sample();
...
SimState.prototype.__sampleFloor = function(room, opts, target) {
    // console.time('__sampleFloor');
    var scope = this;
    var validSampleFun = function(x) {
        return scope.isPositionAgentCanStandAt(x.worldPoint);
    };

    var floorSamples = SceneUtil.sampleRoomFloor(
        room, opts.nsamples, {skipUVColors : true, rng : opts.rng});
    ...
    if (floorSamples && floorSamples.length > 0) {
        ...
        if (validFloorSample) {
            var res = validFloorSample;
        }
    }
    // console.timeEnd('__sampleFloor');
    return res;
};
 ...
 * @param opts.maxIters {int} Number of iterations to try for sampling rooms and
 * floor
 * @param opts.rng {Math.RNG} Random number generator to use for sampling
 * @param [opts.filterRooms] {function(THREE.Object3D): boolean} Optional filter
 * when selecting rooms
 * @returns {{room: *, floorSample: *}}
 * @private
 */
SimState.prototype.__sampleRoomFloor = function(opts) {
    var rooms =
        this.sceneState.getRoomsOrHouseRegions(this.level, opts.filterRooms);
    ...
    if (!validFloorSample) {
        console.warn('Cannot find good position after ' + opts.maxIters +
                     '.  Giving up!');
    }
    if (floorSample) {
...
SimState.prototype.__updateNavSceneSample = function(sample, rng) {
    ...
    sample.position.y +=
        this.agent.originHeight * Constants.metersToVirtualUnit;
    ...
    if (this.agent.angularResolution !== undefined) {
        angle = Math.round(angle / this.agent.angularResolution) *
                this.agent.angularResolution;
    }
    sample.angle = angle;
    return sample
};
...
SimState.prototype.getObjectInfos = function(objectIds) {
    ...
    return objectIds.map(function(id) {
        var node = scope.sceneState.findNode(function(x) {
            return x.userData.id === id;
        });
...
SimState.prototype.getSummary = function() {
    ...
    var summary = {
        ...
        shortestPath : this.navscene.getShortestPath()
    };
    return summary;
};
