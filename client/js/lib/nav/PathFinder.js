...
function SearchOptions(starts, exit_now, sort_key) {
    ...
    // - allow_reprioritize - true in general, but needs to be set to false for
    // greedy best first search (ugly hack)
    this.allow_reprioritize = true;
    this.exit_now = this.exit_now || (function(_) { return false; });
    this.sort_key =
        this.sort_key || (function(id, node) { return node.cost_so_far; });
}

SearchOptions.getSearchOptions = function(graph, starts, exit, algorithm,
                                          early_stop, heuristic) {
    var searchOpts;
    var stopCondition = (early_stop && exit) ? function(ss) {
        return exit.id === ss.current;
    } : null;
    switch (algorithm) {
    case 'BFS':
        searchOpts =
            new SearchOptions(starts, stopCondition,
                              function(id, node) { return node.visit_order; });
        searchOpts.allow_reprioritize = false;
        break;
    case 'DFS':
        searchOpts =
            new SearchOptions(starts, stopCondition, function(id, node) {
...
function octile_grid2d_heuristic(graph, goal, current) {
    ...
    var dx = Math.abs(xy0[0] - xy1[0]);
    var dy = Math.abs(xy0[1] - xy1[1]);
    // console.log('h', xy0, xy1, dx, dy, Math.max(dx,dy) +
    // sqrt2minus1*Math.min(dx,dy));
...
PathFinder.prototype.search = function(starts, end, graph, map, searchState,
                                       heuristic) {
    var options = SearchOptions.getSearchOptions(graph, starts, {id : end},
                                                 'A*', true, heuristic);
    ...
    while (heap.size() > 0) {
        ...
        s.neighbors.forEach(function(next) {
            var new_cost_so_far = (map[s.current].cost_so_far +
                                   graph.edgeWeight(s.current, next));
            if (!map[next]) {
                map[next] = {id : next, in_heap : false};
            }
            if (!map[next].visited ||
                (options.allow_reprioritize && map[next].visited &&
                 new_cost_so_far < map[next].cost_so_far)) {

                map[next].steps = map[s.current].steps + 1;
                map[next].cost_so_far = new_cost_so_far;
                map[next].distance =
                    map[s.current].distance + graph.distance(s.current, next);
